...
#include <stdlib.h>
#include <string.h>
#include <strings.h> /* dwunw-added: for strcasecmp */
...
#include "memleak.h"
#include "memleak_dwunw_events.h" /* dwunw-added: shared event layout */
#include "memleak_dwunw.skel.h" /* dwunw-added: generated skeleton */
#include "trace_helpers.h"
#include "dwunw/dwunw_api.h" /* dwunw-added: libdwunw integration */
#include "dwunw/unwind.h" /* dwunw-added: libdwunw frame capture */

#define DWUNW_ARRAY_SIZE(x) (sizeof(x) / sizeof((x)[0]))
...
static size_t clamp_to_top_stacks(size_t count)
{
	if (env.top_stacks <= 0)
		return 0;

	const size_t limit = (size_t)env.top_stacks;
	return count < limit ? count : limit;
}

/* dwunw-added: CLI-selectable DWARF unwinder modes */
enum dwunw_mode {
	DWUNW_MODE_OFF = 0,
	DWUNW_MODE_FALLBACK,
	DWUNW_MODE_FORCE,
...
static const char *const dwunw_mode_text[] = {
	[DWUNW_MODE_OFF] = "off",
	[DWUNW_MODE_FALLBACK] = "fallback",
	[DWUNW_MODE_FORCE] = "force",
};

struct dwunw_runtime {
	enum dwunw_mode mode;
	struct dwunw_context ctx;
	bool ctx_ready;
	struct ring_buffer *rb;
};

static struct dwunw_runtime dwunw_rt = {
	.mode = DWUNW_MODE_FALLBACK,
	.ctx_ready = false,
	.rb = NULL,
};
...
/* dwunw-added: forward declarations for DWARF unwinder hookups */
static enum dwunw_mode parse_dwunw_mode(const char *arg);
static int setup_dwunw_runtime(struct memleak_dwunw_bpf *skel);
static void teardown_dwunw_runtime(void);
static int handle_dwunw_event(void *ctx, void *data, size_t data_sz);
static void dwunw_maybe_poll(void);
static size_t clamp_to_top_stacks(size_t count);
...
static void disable_kernel_node_tracepoints(struct memleak_dwunw_bpf *skel);
static void disable_kernel_percpu_tracepoints(struct memleak_dwunw_bpf *skel);
static void disable_kernel_tracepoints(struct memleak_dwunw_bpf *skel);

static int attach_uprobes(struct memleak_dwunw_bpf *skel);
...
"./memleak --dwunw-mode=force\n"
"        Emit DWARF-based stacks via libdwunw alongside memleak summaries\n"
"";
...
static const struct argp_option argp_options[] = {
	...
	{"dwunw-mode", 'W', "MODE", 0, "dwunw unwinding mode: off|fallback|force", 0 }, /* dwunw-added */
	{},
};
...
struct syms_cache *syms_cache;
	...
	skel->rodata->dwunw_enabled = dwunw_rt.mode != DWUNW_MODE_OFF; /* dwunw-added */
	...
	ret = memleak_dwunw_bpf__load(skel);
	if (ret) {
	...
	ret = memleak_dwunw_bpf__attach(skel);
	if (ret) {
	...
	if (setup_dwunw_runtime(skel)) { /* dwunw-added */
		ret = -1;
		goto cleanup;
	}
...
cleanup:
	...
	if (ksyms)
		ksyms__free(ksyms);
#endif
	teardown_dwunw_runtime(); /* dwunw-added */
	memleak_dwunw_bpf__destroy(skel);
...
error_t argp_parse_arg(int key, char *arg, struct argp_state *state)
{
	...
	case 'S':
		...
		break;
	case 'W':
		dwunw_rt.mode = parse_dwunw_mode(arg); /* dwunw-added */
		break;
	case 'T':
...
/* dwunw-added: parse CLI string into dwunw_mode enum */
static enum dwunw_mode parse_dwunw_mode(const char *arg)
{
	if (!arg)
		return DWUNW_MODE_FALLBACK;

	for (size_t i = 0; i < DWUNW_ARRAY_SIZE(dwunw_mode_text); ++i) {
		if (dwunw_mode_text[i] && strcasecmp(arg, dwunw_mode_text[i]) == 0)
			return (enum dwunw_mode)i;
	}

	fprintf(stderr, "unknown dwunw-mode '%s', defaulting to fallback\n", arg);
	return DWUNW_MODE_FALLBACK;
}
...
/* dwunw-added: convert shared snapshot into dwunw_regset */
static void copy_dwunw_regset(struct dwunw_regset *dst,
			       const struct memleak_dwunw_regset_snapshot *src)
{
...
/* dwunw-added: pretty print DWARF frames */
static void dwunw_print_frames(const struct dwunw_frame *frames, size_t count)
{
	for (size_t i = 0; i < count; ++i) {
		const struct dwunw_frame *f = &frames[i];
		printf("  [dwunw] #%zu pc=0x%llx sp=0x%llx ra=0x%llx flags=0x%x module=%s\n",
		       i,
...
/* dwunw-added: ring buffer callback pumping events into dwunw_capture */
static int handle_dwunw_event(void *ctx __attribute__((unused)), void *data, size_t data_sz)
{
	if (data_sz < sizeof(struct memleak_dwunw_event) || !dwunw_rt.ctx_ready)
		return 0;
	...
	snprintf(module_path, sizeof(module_path), "/proc/%u/exe", evt->tgid);
	struct dwunw_unwind_request req = {
		.module_path = module_path,
		.regs = &regset,
		.frames = frames,
		.max_frames = DWUNW_ARRAY_SIZE(frames),
		.options = DWUNW_OPTION_NONE,
	};

	if (dwunw_rt.mode != DWUNW_MODE_OFF) {
		req.pid = (pid_t)evt->tgid;
		req.tid = (pid_t)evt->pid;
	}

	size_t written = 0;
	dwunw_status_t st = dwunw_capture(&dwunw_rt.ctx, &req, &written);
	if (st != DWUNW_OK && req.pid > 0) {
		if (dwunw_rt.mode == DWUNW_MODE_FORCE) {
			fprintf(stderr,
			        "[dwunw] default reader required (mode=force) pid=%u comm=%s err=%d\n",
			        evt->tgid,
			        evt->comm,
			        st);
			return 0;
		...
		if (dwunw_rt.mode == DWUNW_MODE_FALLBACK) {
			fprintf(stderr,
			        "[dwunw] default reader failed pid=%u comm=%s err=%d, retry without reader\n",
			        evt->tgid,
			        evt->comm,
			        st);
			req.pid = 0;
	...
	printf("[dwunw] pid=%u comm=%s frames=%zu\n",
	       evt->tgid,
	       evt->comm,
	       written);
	dwunw_print_frames(frames, written);
	return 0;
}

/* dwunw-added: lazy init dwunw context and ring buffer */
static int setup_dwunw_runtime(struct memleak_dwunw_bpf *skel)
{
	if (dwunw_rt.mode == DWUNW_MODE_OFF)
		return 0;

	if (!dwunw_rt.ctx_ready) {
		dwunw_status_t st = dwunw_init(&dwunw_rt.ctx);
		if (st != DWUNW_OK) {
			fprintf(stderr, "failed to init dwunw context: %d\n", st);
			return -1;
		}
		dwunw_rt.ctx_ready = true;
	}

	if (!dwunw_rt.rb) {
		int map_fd = bpf_map__fd(skel->maps.dwunw_events);
		if (map_fd < 0) {
			fprintf(stderr, "dwunw_events map missing\n");
			return -1;
		}
		dwunw_rt.rb = ring_buffer__new(map_fd, handle_dwunw_event, NULL, NULL);
		if (!dwunw_rt.rb) {
...
/* dwunw-added: release dwunw resources */
static void teardown_dwunw_runtime(void)
{
	if (dwunw_rt.rb) {
		ring_buffer__free(dwunw_rt.rb);
		dwunw_rt.rb = NULL;
	}
	if (dwunw_rt.ctx_ready) {
		dwunw_shutdown(&dwunw_rt.ctx);
		dwunw_rt.ctx_ready = false;
	}
}

/* dwunw-added: poll ring buffer while honoring force mode */
static void dwunw_maybe_poll(void)
{
	if (!dwunw_rt.rb)
		return;
	int err = ring_buffer__poll(dwunw_rt.rb, 10);
	if (err < 0 && err != -EINTR && err != -EAGAIN) {
		fprintf(stderr, "dwunw ring buffer poll err=%d\n", err);
		if (dwunw_rt.mode == DWUNW_MODE_FORCE)
			exiting = 1;
	}
}
...
{
	const size_t depth = env.perf_max_stack_depth > 0 ? (size_t)env.perf_max_stack_depth : 0;
...
int print_outstanding_allocs(int allocs_fd, int stack_traces_fd)
{
	...
	size_t nr_allocs_to_show = clamp_to_top_stacks(nr_allocs);
...
int print_outstanding_combined_allocs(int combined_allocs_fd, int stack_traces_fd)
{
	...
	// get min of allocs we stored vs the top N requested stacks
	nr_allocs = clamp_to_top_stacks(nr_allocs);
