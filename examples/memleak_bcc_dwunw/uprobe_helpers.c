...
static int which_program(const char *prog, char *path, size_t path_sz)
{
    ...
    if (snprintf(cmd, sizeof(cmd), "which %s", prog) >= (int)sizeof(cmd)) {
...
check:
    if (ehdr.e_type == ET_EXEC || ehdr.e_type == ET_DYN) {
        ...
        for (i = 0; i < (int)nhdrs; i++) {
            ...
            if (ret < 0)
                continue;
            ret_addr = (Elf64_Addr)ret;
            if (phdr.p_vaddr <= ret_addr &&
                ret_addr < phdr.p_vaddr + phdr.p_memsz) {
                ret = (off_t)(ret_addr - phdr.p_vaddr + phdr.p_offset);
                goto out;
            }
        }
        ret = -1;
