...
2. **平台抽象层 (PAL)**
   ...
   - 实现 x86_64 版本（基线），stub 出 arm64、mips32，并编写占位单元测试确保接口完整。
   - 定义寄存器快照结构（与 BPF 事件格式兼容），含架构标签与版本号。
...
## 交付里程碑
1. **M1 - Skeleton Ready**：完成阶段 1-2，产出初版库骨架与 x86_64 ops。（状态：已完成，GNU Make 构建与目录骨架就绪）
